...
;    bce=bce, $                   ; [未実装]座標系６
    ...
    ;Kuni 
    sat=sat                         ; spacecraft name in character. e.g., sat='mms' 
...
compile_opt idl2
  ...
  if not keyword_set(infile) then begin
    cd, file_dirname(routine_filepath())
;Kuni    if ~n_elements(infile) then infile = dialog_pickfile(FILTER='19971212_lep_psd_8532.txt')
    if ~n_elements(infile) then infile = '19971212_lep_psd_8532.txt'
  ...
  res = olep.read_lep(infile, TRANGE=trange, sat=sat)
  ; 
