...
;Good data has By/Bx < 3.732   and  Bz/Bx < 3.732
	...
	By2Bx = abs(magmgse.y[*,1]/magmgse.y[*,0])
	Bz2Bx = abs(magmgse.y[*,2]/magmgse.y[*,0])
...
;calculate angles b/t despun spinplane antennas and Bo. 
	...
	for i=0L,n-1 do ang_ey[i] = acos(total([0,1,0]*magmgse.y[i,*])/(bmag[i]))/!dtor
	for i=0L,n-1 do ang_ez[i] = acos(total([0,0,1]*magmgse.y[i,*])/(bmag[i]))/!dtor
...
;	get_data,'rbsp'+probe+'_sfit12_mgse',data=tmpp
...
;	store_data,'rbsp'+probe+'_sfit12_mgse',data=tmpp
	store_data,'rbsp'+probe+'_efw_esvy_mgse_vxb_removed_spinfit',data=tmpp
