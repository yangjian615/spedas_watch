...
function rbsp_rotate_field_2_vec,waveform,vec,vec2=vec2,efa=efa
	...
	get_data,vec,data=tmp
	vec = tmp
	if keyword_set(vec2) then begin
		get_data,vec2,data=tmp
		vec2 = tmp
	endif
	...
	Vecmag = sqrt(vec.y[*,0]^2 + vec.y[*,1]^2 + vec.y[*,2]^2)
	if is_struct(vec2) then Vec2mag = sqrt(vec2.y[*,0]^2 + vec2.y[*,1]^2 + vec2.y[*,2]^2)
	...
	if ~keyword_set(efa) and ~keyword_set(vec2) then begin
		...
		for j=0L,nchunks-1 do begin
			...
			vals = rbsp_min_var_rot(wf.y[s:e,*],bkg_field=vec.y[j,*],/nomssg)
