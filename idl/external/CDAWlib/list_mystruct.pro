   ...
   if q[0] eq -1 then begin  ;  Avoid repeating vars already listed
      ...
      if qq[0] ne -1 and qqq[0] ne -1 then begin
	    ...
	    if q[0] ne -1 then begin  ; if, for whatever reason, this var is not
	       ;  included in the input structure, then skip it
...
if string(tmp_stuff(0)) ne '' then begin      
   ...
   case size_tmp(0) of
      ...
      2: begin
	    ...
	    tol=5
	    if (((ts1[0] eq ts2[0]+tol) or (ts1[0] eq ts2[0]-tol)) or $
	       ((ts1[1] eq ts2[1]+tol) or (ts1[1] eq ts2[1]-tol)) or $
	       ((ts1[3] eq ts2[3]+tol) or (ts1[3] eq ts2[3]-tol))) then begin
...
case convar of
   0 : begin
       ...
       if mega_num gt 1 then printf, unit,' **************************************************************************************'
       if mega_num gt 1 then printf, unit, format='(15x)'
   end  ; end case 0
   ;
...
for k=0L, length-1 do begin
   if (ep16) then CDF_EPOCH16,dat(k), yr, mo, dy, hr, mn, sc, milli, micro, nano, pico, /break else $
...
;         return, 0
   ...
   for i=0,ns_tags-1 do begin
      ...
      if (c.(i).cdftype eq 'CDF_EPOCH16') then format='A35' 
