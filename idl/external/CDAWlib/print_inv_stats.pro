...
;$Header: /home/cdaweb/dev/control/RCS/print_inv_stats.pro,v 1.3 2012/05/17 17:22:30 johnson Exp johnson $
;$Locker: johnson $
;$Revision: 15739 $
;-------------------------------------------------------------------------
...
for num_days = 0, ntimes-1 do begin
  ...
  if (d_count gt 0) then begin
    ...
    if (d_count ge 121 and d_count le 140) then num_ds_day[num_days] = 140
    if (d_count ge 141 and d_count le 160) then num_ds_day[num_days] = 160
    if (d_count ge 161 and d_count le 180) then num_ds_day[num_days] = 180
    if (d_count ge 181 and d_count le 200) then num_ds_day[num_days] = 200
    if (d_count ge 201 and d_count le 220) then num_ds_day[num_days] = 220
    if (d_count ge 221 and d_count le 240) then num_ds_day[num_days] = 240
    if (d_count ge 241 and d_count le 260) then num_ds_day[num_days] = 260
    if (d_count ge 261 and d_count le 280) then num_ds_day[num_days] = 280
    if (d_count ge 281 and d_count le 300) then num_ds_day[num_days] = 300
    if (d_count gt 300 ) then num_ds_day[num_days] = 320
...
for y = 0, num_years-1 do begin
  ...
  found = where(num_ds_day eq 140, ds_count) & printf, unit, 'num days where 121-140 datasets = ',ds_count
  found = where(num_ds_day eq 160, ds_count) & printf, unit, 'num days where 141-160 datasets = ',ds_count
  found = where(num_ds_day eq 180, ds_count) & printf, unit, 'num days where 161-180 datasets = ',ds_count
  found = where(num_ds_day eq 200, ds_count) & printf, unit, 'num days where 181-200 datasets = ',ds_count
  found = where(num_ds_day eq 220, ds_count) & printf, unit, 'num days where 201-220 datasets = ',ds_count
  found = where(num_ds_day eq 240, ds_count) & printf, unit, 'num days where 221-240 datasets = ',ds_count
  found = where(num_ds_day eq 260, ds_count) & printf, unit, 'num days where 241-260 datasets = ',ds_count
  found = where(num_ds_day eq 280, ds_count) & printf, unit, 'num days where 261-280 datasets = ',ds_count
  found = where(num_ds_day eq 300, ds_count) & printf, unit, 'num days where 281-300 datasets = ',ds_count
  found = where(num_ds_day eq 320, ds_count) & printf, unit, 'num days where >300 datasets = ',ds_count
