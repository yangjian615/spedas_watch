...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/trunk/spedas/gui/panels/load_plugins/spd_ui_load_fast_data.pro $
;
;--------------------------------------------------------------------------------
...
pro spd_ui_load_fast_data,tabid,loadedData,historyWin,statusBar,treeCopyPtr,timeRangeObj,callSequence,loadTree=loadTree,timeWidget=timeWidget
  ...
  ;TODO: If we add more instrument types, this label should be posted to the statusbar, rather than having this static listing
 ...
 timeWidget = spd_ui_time_widget(selectionBase,$
  ...
  datatypes = ['DeltaB_DSC','DeltaB_GEI','DeltaB_SM','DeltaB_FAC','DeltaB_FAC_V','DeltaB_FAC_SP','B_GEI','DEL_MAG_FLAG','Torquer','Tweaker','MAG_QUAL_FLAG','B_DSC','B_SSC','Bharmonic_DSC','Spin_Freq','Spin_Phase','Orbit','pos_gei','vel_gei','B_model_gei','alt','lat','lng','ilat','ilng','mlt','flat','flng','B_foot_gei','sun_dir_gei','dip_orient_gei','Orbit_Value','Coupling_Matrix','Offsets','Spin_Axis','Spin_Phase_Delta']
  paramArray = ptrarr(1)
  paramArray[0] = ptr_new(ptrarr(1))
  (*paramArray[0])[0] = ptr_new(['*',datatypes])
