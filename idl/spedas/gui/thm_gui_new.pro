...
;$LastChangedDate: 2014-02-10 15:47:40 -0800 (Mon, 10 Feb 2014) $
;$LastChangedRevision: 14262 $
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/trunk/spedas/gui/thm_gui_new.pro $
;-----------------------------------------------------------------------------------
...
;    END
    ...
    'LOADCDAWEB': BEGIN
      thm_ui_spdfcdawebchooser, historyWin=info.historyWin, GROUP_LEADER = info.master,timeRangeObj=info.loadtr
    END
    
    'LOADCDF': BEGIN
      thm_ui_load_themis_cdf, info
    END
...
;      drawWin = info.drawWin
     ...
     thm_ui_init_fileconfig, info.master, info.historyWin  

;      do not erase in case graphcs are reinstalled
;      thm_ui_init_fileconfig, info.master, drawId, drawWin, $ 
;                               info.drawObject, $
...
;Note on code related to various tracking switches
   ...
   'TRACKH': BEGIN
    ...
    'LEGEND': thm_ui_legend_options, info, tlb_statusbar=info.statusBar
...
; created by THM_UI_WINDOW_MENUS__DEFINE
    'WINBUTTON': BEGIN
      result = Widget_Info(event.id, /Button_Set)
...
;      result = dialog_message(help_arr, title = 'THEMIS: GUI Help', /information)
    ...
    'HELPABOUT': thm_ui_help_about, info.master, info.historyWin
...
PRO thm_gui_new,reset=reset,template_filename=template_filename
 ...
 ;Return if there is already a thm_gui_new widget running
  ...
  gui_title = 'THEMIS Data Analysis Software (TDAS)'
...
;    Sensitive=0)
  ...
  loadCDFMenu = Widget_Button(fileMenu, Value='Load THEMIS CDF', UValue='LOADCDF')
...
;  ; Plugins Pull Down Menu (preliminary - leave commented out)
;  
;  pluginsMenu = widget_button(bar, value='Plugins ', /menu)
;  
;  thm_ui_plugin_menu, pluginsMenu 
...
;    Sensitive=0, UValue='REDO')
  ...
  zoomSpinner = thm_ui_spinner(zoomBase, Increment=10, Value=100, uval='ZOOM',UNITS='%', min_value=1, max_value=200, /disable_all_events)
  ...
  drawID = WIDGET_DRAW(graphBase,/scroll,xsize=xsize,ysize=ysize,x_scroll_size=x_scr_size,y_scroll_size=y_scr_size,Frame=3, Motion_Event=1, $
    /Button_Event,keyboard_events=2,graphics_level=2,renderer=!THM_GUI_NEW.renderer,retain=retain,/expose_events,/tracking_events,/viewport_events)
...
;    UValue='MARKERP')
  ...
  fieldModelSettings = obj_new('THM_UI_FIELDMODELS_SETTINGS')
