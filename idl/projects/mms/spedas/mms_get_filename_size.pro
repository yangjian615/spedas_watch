...
function mms_get_filename_size, json_object
    ...
    num_structs = n_elements(json_object)/3
    counter = 0
    remote_file_info = replicate({filename: '', filesize: 0l}, num_structs)

    for struct_idx = 0, n_elements(json_object)-1, 3 do begin
        remote_file_info[counter].filename = (strsplit(json_object[struct_idx], '": "', /extract))[2]
        remote_file_info[counter].filesize = (strsplit((strsplit(json_object[struct_idx+2], '": "', /extract))[1], '}', /extract))[0]
        
        ; remote_file_info[counter].timetag = (strsplit((strsplit(json_object[struct_idx+1], '": "', /extract))[1], '}', /extract))[0]
        counter += 1
    endfor
