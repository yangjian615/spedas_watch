...
;PURPOSE:
; given a MMS tplot variable name, check to see if spinras and spindec are available, if they do not, load
; the state data for the appropriate time period
;CALLING SEQUENCE:
; mms_autoload_support, vname=vname, spinaxis=spinaxis, slp=slp, probe_in=probe,
...
;
;-
Pro mms_autoload_support, vname=vname, history_out=history_out, probe_in=probe_in, trange=trange, progobj=progobj, _extra=_extra
  ...
  If (loadeph) Then Begin
    If (obj_valid(progobj)) Then progobj -> update, 0.0,  $
      text = 'Loading ephemeris data for Calibration, Probe: '+probe
    mms_load_state, probe = probe_in, trange = trange, /ephemeris_only
    tj = time_string(trange)        ;for history
    history_out = 'thm_load_state, probe = '+''''+probe_in+''''+$
      ', trange = ['+''''+tj[0]+''''+', '+''''+tj[1]+''''+$
      '], /ephemeris_only'
    If(obj_valid(progobj)) Then progobj -> update, 100.0,  $
      text = 'Finished Loading ephemeris data for Calibration, Probe: '+probe_in
  endif
  If (loadatt) Then Begin
    If (obj_valid(progobj)) Then progobj -> update, 0.0,  $
      text = 'Loading attitude data for Calibration, Probe: '+probe
