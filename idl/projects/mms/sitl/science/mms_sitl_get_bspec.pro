...
pro mms_sitl_get_bspec, start_date, end_date, cache_dir, data_status, sc_id=sc_id, no_update = no_update
  ...
  if login_flag eq 0 or file_flag eq 0 then begin
    ...
    cdf_struct = mms_open_dsp_acb_cdf(files_open(0))
    times = cdf_struct.x
    b1 = cdf_struct.b1
    b2 = cdf_struct.b2
    b3 = cdf_struct.b3
    ...
    if n_elements(files_open) gt 1 then begin
      for i = 1, n_elements(files_open)-1 do begin
        temp_struct = mms_open_dsp_acb_cdf(files_open(i))
    ...
    sdpvarnamex = sc_id + '_dsp_sitl_bpsd_1'
    sdpvarnamey = sc_id + '_dsp_sitl_bpsd_2'
    sdpvarnamez = sc_id + '_dsp_sitl_bpsd_3'
