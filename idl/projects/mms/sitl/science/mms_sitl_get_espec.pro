...
pro mms_sitl_get_espec, start_date, end_date, cache_dir, data_status, sc_id=sc_id, no_update = no_update
  ...
  if login_flag eq 0 or file_flag eq 0 then begin
    ...
    cdf_struct = mms_open_dsp_ace_cdf(files_open(0))
    times = cdf_struct.x
    ex = cdf_struct.ex
    ey = cdf_struct.ey
    ez = cdf_struct.ez
    ...
    if n_elements(files_open) gt 1 then begin
      for i = 1, n_elements(files_open)-1 do begin
        temp_struct = mms_open_dsp_ace_cdf(files_open(i))
    ...
    sdpvarnamex = sc_id + '_dsp_sitl_epsd_x'
    sdpvarnamey = sc_id + '_dsp_sitl_epsd_y'
    sdpvarnamez = sc_id + '_dsp_sitl_epsd_z'
