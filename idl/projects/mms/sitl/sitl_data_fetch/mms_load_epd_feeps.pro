pro mms_load_epd_feeps, sc=sc, no_update = no_update, reload = reload
...
for j = 0, n_elements(sc)-1 do begin
  ...
  if keyword_set(no_update) then begin
    mms_data_fetch, local_flist, login_flag, download_fail, sc_id=sc(j), $
      instrument_id='feeps', mode=mode, optional_descriptor='electron', $
  ...
  if login_flag eq 1 then begin
    ...
    mms_check_local_cache, local_flist, file_flag, $
      mode, 'feeps', level, sc(j), optional_descriptor='electron'
  endif
  ...
  if login_flag eq 0 or file_flag eq 0 then begin
    ...
    feeps_struct = mms_sitl_open_feeps_cdf(files_open(0))
    times = feeps_struct.times
    elec_t1 = feeps_struct.elec_t1
    elec_t1_name = feeps_struct.elec_t1_name
    ...
    print, 'No feeps data available locally or at SDC or invalid query!'
  endelse
