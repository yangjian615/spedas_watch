FUNCTION eva_data_load_mms, state
  ...
  for i=0,imax-1 do begin; for each requested parameter
    ...
    if strmatch(paramlist[i],'*_bvec*') then begin
    ...
    if strmatch(paramlist[i],'*_dce*') then begin
      mms_sitl_get_dce,  state.start_time, state.end_time, cache_dir, sdp_status, sc_id=sc;, coord=coord, no_update = no_update
