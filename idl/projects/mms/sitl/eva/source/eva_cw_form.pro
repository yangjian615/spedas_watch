; $Id: eva_cw_form.pro 16847 2015-02-04 08:30:08Z moka $
;
...
;
; PURPOSE:
; EVA_CW_FORM is a compound widget that simplifies creating
...
;
; CALLING SEQUENCE:
; widget = EVA_CW_FORM([Parent,] Desc)
;
; INPUTS:
...
;   and its value returned as the result of EVA_CW_FORM.  For non-
...
;  *****************
;
;    To use EVA_CW_FORM inside another widget:
;     a = widget_base(title='Testing')
;     b = EVA_CW_form(a, desc, /COLUMN)
...
;    ID              LONG                <id of EVA_CW_FORM widget>
...
function EVA_CW_FORM_PARSE, Extra, Name, Value, Index=Index
...
pro EVA_CW_FORM_APPEND, extra, e, keyword, USE_VALUE=use_value, ACTUAL_KEYWORD=akw
  COMPILE_OPT hidden
  
  if EVA_CW_FORM_PARSE(e, keyword, value) then begin
...
pro EVA_CW_FORM_LABEL, parent, nparent, e, frame
  ;Put LABEL_LEFT and/or LABEL_RIGHT on a base.
  
  COMPILE_OPT hidden
  
  nparent = parent
  if EVA_CW_FORM_PARSE(e, 'LABEL_LEFT', value) then begin
    nparent = WIDGET_BASE(nparent, /ROW, FRAME=frame)
    frame = 0
    junk1 = WIDGET_LABEL(nparent, VALUE=value)
  endif
  if EVA_CW_FORM_PARSE(e, 'LABEL_TOP', value) then begin
...
pro EVA_CW_FORM_BUILD, parent, desc, cur, ids, lasttag
  ...
  while cur lt n do begin
    ...
    if n_elements(a) gt 3 then begin  ;Addt'l common params?
      ...
      quit = EVA_CW_FORM_PARSE(e, 'QUIT')
      frame = EVA_CW_FORM_PARSE(e, 'FRAME')
      efn = EVA_CW_FORM_PARSE(e, 'EVENT', event_fun)
      EVA_CW_FORM_APPEND, extra, e, 'FONT', /USE_VALUE
      EVA_CW_FORM_APPEND, extra, e, 'COLUMN'
      EVA_CW_FORM_APPEND, extra, e, 'ROW'
      EVA_CW_FORM_APPEND, extra, e, 'LEFT', ACTUAL='ALIGN_LEFT'
      EVA_CW_FORM_APPEND, extra, e, 'CENTER', ACTUAL='ALIGN_CENTER'
      EVA_CW_FORM_APPEND, extra, e, 'RIGHT', ACTUAL='ALIGN_RIGHT'
    endif else e = ''
    ...
    case STRUPCASE(a[1]) of   ;Which widget type?
      ...
      ENDCASE
      'BUTTON': BEGIN
        EVA_CW_FORM_APPEND, extra, e, 'LABEL_LEFT', /USE_VALUE
        EVA_CW_FORM_APPEND, extra, e, 'LABEL_TOP', /USE_VALUE
        exclusive = EVA_CW_FORM_PARSE(e,'EXCLUSIVE')
        no_release = EVA_CW_FORM_PARSE(e,'NO_RELEASE')
        values = strtok(a[2],'|', /EXTRACT, ESC='\')
        if n_elements(values) ge 2 then begin
          type = 0
          if EVA_CW_FORM_PARSE(e, 'SET_VALUE', temp) then begin
      ...
      ENDCASE
      'DROPLIST': BEGIN
        EVA_CW_FORM_LABEL, parent, nparent, e, frame
        new = WIDGET_DROPLIST(nparent, VALUE = strtok(a[2], '|',/EXTRACT), $
          FRAME=frame, UVALUE=ids[n], _EXTRA=extra)
        if EVA_CW_FORM_PARSE(e, 'SET_VALUE', value) then begin
      ...
      'INTEGER': BEGIN
        ...
        uextra = { VALUE: value }
        EVA_CW_FORM_LABEL, parent, nparent, e, frame
        if EVA_CW_FORM_PARSE(e, 'WIDTH', temp) then width = fix(temp) else width=6
        if EVA_CW_FORM_PARSE(e, 'YSIZE', temp) then ysize = fix(temp) else ysize=1
        new = WIDGET_TEXT(nparent, /ALL_EVENTS, /EDITABLE, YSIZE=ysize, $
      ...
      'LIST': BEGIN
        ...
        if EVA_CW_FORM_PARSE(e, 'HEIGHT', temp) eq 0 then temp = n_elements(v)
    ...
    if type ge 0 then begin
      if EVA_CW_FORM_PARSE(e, 'TAG', value) then value = STRUPCASE(value) $
    ...
    if dflags and 1 then EVA_CW_FORM_BUILD, new, desc, cur, ids, lasttag  ;Begin new
    if (dflags and 2) ne 0 then return  ;End current
  endwhile
end       ;EVA_CW_FORM_BUILD



Function EVA_CW_FORM_EVENT, ev    ;Event handler for EVA_CW_FORM
  COMPILE_OPT hidden
  ...
  return, ret
end     ;EVA_CW_FORM_EVENT


Pro EVA_CW_FORM_SETV, id, value ;In this case, value = { Tagname : value, ... }
  COMPILE_OPT hidden
...
Function EVA_CW_FORM_GETV, id ;Return value of a EVA_CW_FORM widget.
  COMPILE_OPT hidden
...
pro EVA_CW_form_modal_event, ev
  COMPILE_OPT hidden
...
FUNCTION EVA_CW_FORM, parent, desc, $
  ...
  EVA_CW_FORM_BUILD, base, desc, 0, ids, lasttag
  widget_control, base, EVENT_FUNC='EVA_CW_FORM_EVENT', $
    FUNC_GET_VALUE='EVA_CW_FORM_GETV', PRO_SET_VALUE='EVA_CW_FORM_SETV'
    
  if n_params() eq 1 then begin   ;Modal?
    WIDGET_CONTROL, p, /realize
    XMANAGER, 'EVA_CW_FORM', p, EVENT_HANDLER='EVA_CW_FORM_MODAL_EVENT'
    v = TEMPORARY(*handle)
    PTR_FREE, handle
