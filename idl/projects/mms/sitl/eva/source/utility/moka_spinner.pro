;+
; NAME:
;   moka_SPINNER  --> A modified version of thm_ui_spinner
;      (returns event:valid = 0 if just selected)
;      (added input_focus in the set_value procedure)
;
; PURPOSE:
...
;   Use moka_spinner_set_max_value/moka_spinner_set_min_value to change max_value, min_value.
;   moka_spinner_get_max_value/moka_spinner_get_min_value return MAX_VALUE, MIN_VALUE
;
; NOTES:
; (lphilpott 06/10/11)
...
;
;
; Added get and set procedures for min and max designed to be called externally, eg. moka_spinner_set_max_value
...
;
;$LastChangedBy: moka $
;$LastChangedDate: 2015-02-04 00:30:08 -0800 (Wed, 04 Feb 2015) $
;$LastChangedRevision: 16847 $
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/trunk/projects/mms/sitl/eva/source/utility/moka_spinner.pro $
;---------------------------------------------------------------------------------


FUNCTION moka_spinner_button_event, event, down_click=down
  ...
  WHILE (1) do begin
    ...
    new = moka_spinner_num_to_string(new, state.units,state.precision)
    state.value = new
...
FUNCTION moka_spinner_update_event, event
...
;  if  Tag_Names(event,/Structure_Name) eq 'WIDGET_TEXT_CH' && event.ch eq 10 then begin
  if Tag_Names(event,/Structure_Name) ne 'WIDGET_TEXT_SEL' then begin
...
;        widget_control, event.id, set_value=strtrim(string(value),2)+state.units
    ...
    return, {ID: parent, TOP: event.top, HANDLER: event.handler, VALUE:double(new), VALID: 0}
  endelse
...
FUNCTION moka_spinner_up_click, event

  compile_opt idl2, hidden
  RETURN, moka_spinner_button_event(event)
  
END ;---------------------------------------------------------------------



FUNCTION moka_spinner_down_click, event

  compile_opt idl2, hidden
  RETURN, moka_spinner_button_event(event, /down_click)
  
END ;---------------------------------------------------------------------



FUNCTION moka_spinner_getvalue, base
...
PRO moka_spinner_setvalue, base, value
  ...
  widget_control,text,get_uvalue=state
  state.value = moka_spinner_num_to_string(value,state.units,state.precision)
  widget_control, text, SET_VALUE=state.value,set_text_select=offset,input_focus=1
  
  ;Update spinner value with event handler
  set = moka_spinner_update_event( {ID: text, TOP: base, HANDLER: text} )
  
END ;---------------------------------------------------------------------



FUNCTION moka_spinner_num_to_string, num_in, units_in,precision
...
PRO moka_spinner_remove_spaces, string_in
...
pro moka_spinner_set_max_value, base, maxvalue
  text = widget_info(base, FIND_BY_UNAME='_text')
  widget_control,text,get_uvalue=state
...
pro moka_spinner_set_min_value, base, minvalue
  text = widget_info(base, FIND_BY_UNAME='_text')
  widget_control,text,get_uvalue=state
...
pro moka_spinner_get_max_value, base, maxvalue
  text = widget_info(base, FIND_BY_UNAME='_text')
  widget_control,text,get_uvalue=state
  maxvalue = state.maxValue
end

pro moka_spinner_get_min_value, base, minvalue
  text = widget_info(base, FIND_BY_UNAME='_text')
  widget_control,text,get_uvalue=state
...
FUNCTION moka_spinner, parent,          $
  ...
  value =  moka_spinner_num_to_string(valuenum, units,precision)
  ...
  base = widget_base(parent, /ROW,            $
    FUNC_GET_VALUE='moka_spinner_getvalue',        $
    PRO_SET_VALUE='moka_spinner_setvalue',         $
  ...
  if undefined(disable_all_events) then begin
      text = widget_text(base, /editable, /all_events,              $
        EVENT_FUNC='moka_spinner_update_event',                         $
  ...
  ; 'Up' Button
  up_button = widget_button(button_base, EVENT_FUNC='moka_spinner_up_click',    $
    /BITMAP, VALUE= filepath('spinup.bmp', SUBDIR=['resource','bitmaps']), $
    /PUSHBUTTON_EVENTS, UNAME='_up',UVALUE=0, XSIZE=16+one, YSIZE=10+one, $
    tooltip=tooltip)
    
  ; 'Down' Button
  down_button = widget_button(button_base, EVENT_FUNC='moka_spinner_down_click', $
