...
;     eic_read_config
;PURPOSE:
;     Reads the eic configuration file
;CALLING SEQUENCE:
;     eic_struct = eic_read_config()
;INPUT:
...
;OUTPUT:
; otp = a structure with the changeable fields of the !eic
;           structure
...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/trunk/projects/eic/eic_read_config.pro $
;-
Function eic_config_template
  ...
  Return, ppp
End
Function eic_read_config, header = hhh
  otp = -1
;First step is to get the filename
  dir = eic_config_filedir(/app_query)
...
;check for slashes, add if necessary
  temp_string = strtrim(!eic.local_data_dir, 2)
  ll = strmid(temp_string, strlen(temp_string)-1, 1)
  If(ll Ne '/' And ll Ne '\') Then temp_string = temp_string+'/'
  !eic.local_data_dir = temporary(temp_string)
  temp_string = strtrim(!eic.remote_data_dir, 2)
  ll = strmid(temp_string, strlen(temp_string)-1, 1)
  If(ll Ne '/' And ll Ne '\') Then temp_string = temp_string+'/'
  !eic.remote_data_dir = temporary(temp_string)
