...
qq = where((tdiff2 lt 1.0) and (tdiff2 gt 0.1), nqq) +1.  ;for a clock jump, we're looking for values of ~0.499xxx. Add 1 so that qq is the "trouble packet" rather than one before it
...
if nqq ge 1 then begin  ;if there's a clock jump
      ...
      for jj = 0, n_elements(qq)-1 do begin
            if SC[packet_arr[qq[jj]]] eq (SC[packet_arr[qq[jj]-1]]+1) then begin  ;no packet is missing so jump is clock jitter
                  ;No packets missing therefore subtract the added 0.49 s. 
                  ...
                  if s_ticks gt sc_clk2[packet_arr[qq[jj]]] then begin
                      rem_ticks = abs(sc_clk2[packet_arr[qq[jj]]] - s_ticks)  ;left over sub ticks
                      sc_clk1[packet_arr[qq[jj]]] = sc_clk1[packet_arr[qq[jj]]] - 1.D  ;subtract 1 sec
                      sc_clk2[packet_arr[qq[jj]]] = sc_clk2[packet_arr[qq[jj]]] - rem_ticks  ;subtract remaining sub ticks
                  
                  endif else sc_clk2[packet_arr[qq[jj]]] = sc_clk2[packet_arr[qq[jj]]] - s_ticks  ;subtract subticks
