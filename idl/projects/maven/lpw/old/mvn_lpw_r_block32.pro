...
;----------------------
; history:
; New version of decompression by Laila
; last changes: 2012 April 24 by: Laila
;----------------------
;
...
function mvn_lpw_r_block32,data,ptr,nn_e,mask16,bin_c,index_arr,arr_size,edac_on
  ...
  ; print,'##block32### ',nii,n_loops,ptr,ptr_end,n_elements(data),' x ',ptr_size,n_elements(tmp)
 ...
 ;  if edac_on EQ 1 or reject EQ 1 then begin
 ;  print,'Error ',edac_on,reject,' nn_e ',nn_e  
 ;  print,'0 to 15 numbers :',bin[0+nn_e:15+nn_e]
 ;  print,'16 to 22 numbers :',bin[16+nn_e:22+nn_e]
 ;  print,'23 to 40 numbers :',bin[23+nn_e:40+nn_e]   
 ;  endif
         nd=nd_1+1   ; the recorded information is ND-1   
         nn_g=7+16+nn_e
   
   
  ;        print,'BB temp ',0,nii,nd,ptr,nn_g,' X ',bin(0+nn_e),' ',temp_32(0)          ,' x ',abs(temp_32(0)),' F ',bin(0+nn_e:15+nn_e),' EN ',bin(16+nn_e:22+nn_e),  $
  ;                         format='(A,5I,A,I,A,F,A,B16,A,16I1,A,7I1)'           
   if nn_g+31*nd GT n_elements(bin) then nd= (n_elements(bin)-nn_g)/31
  ;         print,'BC temp ',0,nii,nd,ptr,nn_g,' X ',bin(0+nn_e),' ',temp_32(0)          ,' x ',abs(temp_32(0)),' F ',bin(0+nn_e:15+nn_e),' EN ',bin(16+nn_e:22+nn_e),  $
  ;                         format='(A,5I,A,I,A,F,A,B16,A,16I1,A,7I1)'         
 ...
 ;                        (1-bin[nn_g+0+ii*nd])* total(double(                                       bin[nn_g+ii*nd+1:nn_g+ii*nd+15] *bin_c[14-index_arr[0:14]]) ) -$                                        ; if it is positive
 ;                           bin[nn_g+0+ii*nd] *(total(double((1-bin[nn_g+ii*nd+1:nn_g+ii*nd+15])*(1+bin[nn_g+ii*nd+1:nn_g+ii*nd+15])*bin_c[14-index_arr[0:14]]) )+1) $ ; if it is negative       
...
; if nd EQ 16 then  print,'########## ',n_elements(temp_32)  
;  if nd EQ 16 then $
;     for ii=0,30 do $
;     print,'### nd=16 ',ii,1+ii*nd+nn_g,15+ii*nd+nn_g,(15+ii*nd+nn_g)-(1+ii*nd+nn_g), $
;        (1-bin[nn_g+0+ii*nd])* total(double(                                        bin[1+ii*nd+nn_g:15+ii*nd+nn_g]*bin_c[14-index_arr[0:14]])), $
;     -1.0* bin[nn_g+0+ii*nd]  *(total(double((1-bin[1+ii*nd+nn_g:15+ii*nd+nn_g])*(1+bin[1+ii*nd+nn_g:15+ii*nd+nn_g])*bin_c[14-index_arr[0:14]]))+1),$
;       ' X ',bin[nn_g+ii*nd:nn_g+(ii+1)*nd-1],' X ',temp_32[ii+1],format='(A,4I,2F,A,16I1,A,F)'  
;  if nd EQ 16 then $
;       plot,temp_32,title='part pkt nd '+strcompress(nd,/remove_all),charsize=1.5
