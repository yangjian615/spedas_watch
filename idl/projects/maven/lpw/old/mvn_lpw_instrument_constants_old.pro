;----------------------
;
;   pro mvn_lpw_instrument_constants
;
;----------------------
;
;  This allowes the different constants to be located in one place
;  to keep the history of how they are changed
;  Also allow the different boards to have different calibration numbers
;
;----------------------
;  contains routines/procedures:
;  mvn_lpw_instrument_constants
;----------------------
;example
; to run
;     mvn_lpw_instrument_constants,'EM1'
;----------------------
; history:
; last change: 2011 october 5  by:LA
;----------------------
;
;*******************************************************************
...
case board of
 'EM1': BEGIN
                 ; constants associated with DAC
                                                                                      ;const_DAC_volt = 130./2048.                              ;20110916 EM1 & EM2 DAC conversion factor
...
;                 const_w_bias2_DAC=    -1.0*(85./2048.)
 ...
 'EM3': BEGIN
       ...
       END
 'FM': BEGIN
            print,'(mvn_lpw_instrument_constants) FM not activated yet below from EM1 on october 5'           
                 ; constants associated with DAC                             ;20110916 EM1 & EM2 DAC conversion factor
                 const_lp_bias1_DAC=   -1.0*(50./2048.)
...
new_name_hf=[1,2,4,8,16,32,64,128]                      ;pas_spec number of 1024 FFT's to ave together on E12_HF channel reserve   ; table from ICD 7.12.3
    ...
    nn_euv_diodes=4                                       ;number of diodes
                                                    ;print,'(mvn_lpw_euv) Warning, what is the sampling rate? 1 sec or 0.125 seconds? OR?********'
...
;------------- Put the information into a string  LPW_const-------------
