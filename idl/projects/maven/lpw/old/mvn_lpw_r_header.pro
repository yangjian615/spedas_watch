...
;example
; to run
;     mvn_lpw_r_header, '\\lasp-smb\vannattc\MAVEN\lab_rlse2_20110505.dat',output
;----------------------
; history:
; original file r_header made by corinne vannatta
; last changes: 2012 May 1 by: Laila
;----------------------
;
...
; The data is stored in one long array as well as arrays of each individual type of data.
...
print,'r_header filename ',filename
...
If (keyword_set(packet)) EQ 0 Then packet=['HSK','EUV','AVG','SPEC','HSBM','WPK']   
...
t1=systime(1,/seconds)
    ...
    APID2     = newfile_unsigned(qq)  mod 256          ; this will give us values between ~81- ~97
      ...
      length2           = newfile_unsigned[qq+2]+2  ;packet length
      ...
      qq1               = where(tmp EQ 1,nq1)
 ;     newfile_byte      = reform(newfile_byte(qq1))  ;only used in APID 82,85,86,88
t2=systime(1,/seconds)
...
;i             = 0L
;length        = fltarr(1)
;message_type  = fltarr(1)
;counter_all1  = fltarr(1)
;Getting # headers and lengths for allocation
;change this to save the counter, since that is the basic information
;while counter LT n_elements(newfile_unsigned) do begin
;   if counter EQ 0 then length = [newfile_unsigned(counter+place+2)] else length = [length,newfile_unsigned(counter+place+2)]
;   counter_all1  = [counter_all1,counter]
;   counter       = double(counter + floor(length(i)+1)/2 + 3) + place
;   i             = i + 1
; endwhile
;;print,n_elements(length),n_elements(newfile_unsigned),counter,total(length),length(n_elements(length)-1)
;if counter GE n_elements(newfile_unsigned) then length=length(0:n_elements(length)-2)
;;print,n_elements(length),n_elements(newfile_unsigned),counter,total(length),length(n_elements(length)-1)
;;stanna
...
;if    newfile_unsigned[counter+1]  EQ 49152+2 then $   ;packet counter 1
;if    newfile_unsigned[counter]  EQ 2140 then $   ;apid 0x5c
;if    newfile_unsigned[counter]  EQ 2130 then $   ;apid 0x52
;if    newfile_unsigned[counter]  EQ 2132 then $   ;apid 0x54
;if    newfile_unsigned[counter]  EQ 2141 then $   ;apid 0x5d
;if    newfile_unsigned[counter]  EQ 2134 then $   ;apid 0x56
;if    newfile_unsigned[counter]  EQ 2131 then $   ;apid 0x53
;print,n_elements(counter_all1)-1,counter,newfile_unsigned[counter+place+2],' $$ ',newfile_unsigned[counter],' ',newfile_unsigned[counter+1], $
;' ',newfile_unsigned[counter+2],' ',newfile_unsigned[counter+3],' ',newfile_unsigned[counter+4],' ',newfile_unsigned[counter+5],format='(2I,I,A4,z,A,z04,A,z04,A,z04,A,z04,A,z04)'


str1='(A4,I,A4,'+strcompress(floor(newfile_unsigned[counter+2]+1)/2 + 3,/remove_all)+'z)'

;if    newfile_unsigned[counter]  EQ 2135 then $   ;apid 0x57
;if    newfile_unsigned[counter]  EQ 2130 then $   ;apid 0x51
;if    newfile_unsigned[counter]  EQ 2132 then $   ;apid 0x54
;if    newfile_unsigned[counter]  EQ 2131 then $   ;apid 0x53
;print,'$-> ',n_elements(counter_all1)-1,' ## ',newfile_unsigned[counter:double(counter + floor(newfile_unsigned[counter+place+2]+1)/2 + 3) + place],  format=str1

;if counter GE 62616  then stanna
;if counter GE 33296+10 then print,newfile_unsigned[ 8152:33420],format='(25269z)' 

;if counter GT 33296+10 then stanna
;print, newfile_unsigned[counter] ,' # ', newfile_unsigned[counter+1] 
;if     newfile_unsigned[counter]  EQ 2141 and newfile_unsigned[counter+1]  GT 49152+2 then stanna



  counter=double(counter + floor(newfile_unsigned[counter+place+2]+1)/2 + 3) + place

;if n_elements(counter_all1) GT 4580 then begin
;PAS_S_LF Sequence Count Skipped. seqIndex =       1  SC(seqIndx-1) =      0.00000  SC(seqIndx) =      2.00000
;if n_elements(counter_all1) GT 2780 then begin
;if n_elements(counter_all1) GT 850 then begin
;aa1=4460
;aa2=4580
;stanna
;endif
...
;if total(length-newfile_unsigned(counter_all1+place+2)) NE 0 then stanna
length=newfile_unsigned[counter_all1+place+2]
...
;message_header   = strarr(sizelength(1),16)
;CCSDS_primary    = strarr(sizelength(1))
;CCSDS_secondary  = strarr(sizelength(1),16*3)
DFB_header       = strarr(sizelength(1),16)
vers             = fltarr(sizelength(1))
type             = fltarr(sizelength(1))
SHF              = fltarr(sizelength(1))
APID             = fltarr(sizelength(1))
GF               = fltarr(sizelength(1))
SC               = fltarr(sizelength(1))
length2          = fltarr(sizelength(1))
data             = fltarr(1)
...
;E12_HF_GB        = fltarr(sizelength(1))  re-define below
...
xfer_time     = !values.f_nan
cap_time      = !values.f_nan
htime_type    = !values.f_nan
...
;CLK1_dummy      = 1d
;CLK2_dummy      = 1d
;These are for HSBM concatenation
;hsbm_lf_1pkt    = 0 ;[]
;hsbm_lf         = fltarr(1024) ; []
;hsbm_lf_partpkt = 0.
;hsbm_mf_1pkt    = 0 ;[]
...
t1=systime(1,/seconds)
    ...
    ;this is to make quick conversions
...
for ni=0L, n_elements(length)-1 do begin 
 ; message_header(i,*) = string(newfile_unsigned(counter),format = '(B016)')
 ; dummy = string(newfile_unsigned(counter+place),format = '(B016)') + string(newfile_unsigned(counter+place+1),format = '(B016)') + string(newfile_unsigned(counter+place+2), format = '(B016)')
 ; CCSDS_primary(i) = dummy
 ; reads,dummy, vers_dummy, type_dummy, SHF_dummy, APID_dummy, GF_dummy, SC_dummy, length_dummy, format='(B03, B01, B01, B011, B02, B014, B016)'
 ; reads,dummy, vers_dummy, type_dummy, SHF_dummy, APID_dummy, GF_dummy, SC_dummy, length_dummy2, format='(B03, B01, B01, B011, B02, B014, B016)'
 ; vers(i) = vers_dummy
 ; type(i) = type_dummy
 ; SHF(i) = SHF_dummy
 ; APID(i) = APID_dummy
 ...
 ; length2(i) = length_dummy + 2
  ...
  length2(i)   = length_dummy + 2
 ; if  vers_dummy NE vers(i) then stanna
 ; if  type_dummy NE type(i) then stanna
 ; if  SHF_dummy NE  SHF(i) then stanna
 ; if  APID_dummy NE  APID(i) then stanna
 ; if  GF_dummy NE   GF(i) then stanna
 ; if  SC_dummy NE   SC(i) then stanna
 ; if  length_dummy NE length_dummy2 then stanna
   
 ; dummy2 = string(newfile_unsigned(counter+place+3),format = '(B016)') +  string(newfile_unsigned(counter+place+4), format = '(B016)') +  string(newfile_unsigned(counter+place+5), format = '(B016)')
 ; CCSDS_secondary(i) = dummy2
 ; reads,dummy2,CLK1_dummy,CLK2_dummy,format = '(B032,B016)'
 ; SC_CLK1(i) = double(CLK1_dummy)
 ; SC_CLK2(i) = double(CLK2_dummy)
   tmp        = [[mask16[newfile_unsigned[counter+place+3],*]],[mask16[newfile_unsigned[counter+place+4],*]]]
   SC_CLK1(i) = total(double(tmp[0:31] *bin_c[31-index_arr[0:31]] ))
   tmp        = [mask16[newfile_unsigned[counter+place+5],*]]
   SC_CLK2(i) = total(tmp[0:15] *bin_c[15-index_arr[0:15]] )
 ;if CLK1_dummy NE SC_CLK1(i) then stanna
 ;if CLK2_dummy NE SC_CLK2(i) then stanna
 
  
  ;reads,dummy2,nothing,Course_dummy,CLK2_dummy2, format = '(B016, B016, B016)'
  ;course_clk(i) = double(course_dummy) 
;  DFB_header(i,*) = newfile_unsigned(counter+place+6) 
 ; counter_specific(i) = long(counter)   ; the number used in the routine
 ; counter_all(i)   =  counter     ; the modified number
...
IF total(length2 GT 2048) GT 0 then stanna  ;chrash the program...
...
E12_HF_GB    = fltarr(nn_PAS_HF >1)   ; this is just to make sure this is >1 not nessicarly that there are any package
...
;Spec preallocations   ; I have removed this from output, will it still work??
;if nn_ACT_LF    GT 1 then ACT_S_LF_length = floor(length(where(APID EQ 89))+1)/2+3          else ACT_S_LF_length = 1
;if nn_ACT_MF    GT 1 then ACT_S_MF_length = floor(length(where(APID EQ 90))+1)/2+3          else ACT_S_MF_length = 1
;if nn_ACT_HF    GT 1 then ACT_S_HF_length = floor(length(where(APID EQ 91))+1)/2+3          else ACT_S_HF_length = 1
;if nn_PAS_LF    GT 1 then PAS_S_LF_length = floor(length(where(APID EQ 92))+1)/2+3          else PAS_S_LF_length = 1
;if nn_PAS_MF    GT 1 then PAS_S_MF_length = floor(length(where(APID EQ 93))+1)/2+3          else PAS_S_MF_length = 1
;if nn_PAS_HF    GT 1 then PAS_S_HF_length = floor(length(where(APID EQ 94))+1)/2+3          else PAS_S_HF_length = 1
;if n_elements(ACT_S_LF0) GT 1 then ACT_LF_array    = fltarr(n_elements(ACT_S_LF_length),ACT_S_LF_length(0)-9) else ACT_LF_array = fltarr(1)
;if n_elements(ACT_S_MF0) GT 1 then ACT_MF_array    = fltarr(n_elements(ACT_S_MF_length),ACT_S_MF_length(0)-9) else ACT_MF_array = fltarr(1)
;if n_elements(ACT_S_HF0) GT 1 then ACT_HF_array    = fltarr(n_elements(ACT_S_HF_length),ACT_S_HF_length(0)-9) else ACT_HF_array = fltarr(1)
;if n_elements(PAS_S_LF0) GT 1 then PAS_LF_array    = fltarr(n_elements(PAS_S_LF_length),PAS_S_LF_length(0)-9) else PAS_LF_array = fltarr(1)
;if n_elements(PAS_S_MF0) GT 1 then PAS_MF_array    = fltarr(n_elements(PAS_S_MF_length),PAS_S_MF_length(0)-9) else PAS_MF_array = fltarr(1)
;if n_elements(PAS_S_HF0) GT 1 then PAS_HF_array    = fltarr(n_elements(PAS_S_HF_length),PAS_S_HF_length(0)-9) else PAS_HF_array = fltarr(1)


;counter_all        = fltarr(i)       ; the modified counter
;counter_specific   = lonarr(i)       ; the counter used in the packed
;counter = 0d
;i       = 0L
;while counter LT n_elements(newfile_signed) do begin   ;can this whiel loop be removed??? to the above loop????? why did it not work last time??
;     counter_specific(i) = long(counter)   ; the number used in the routine
;     counter             = double(counter + floor(length(i)+1)/2 + 3) -1 + place
;     counter_all(i)      =  long(counter)     ; the modified number
;     i                   = i + 1    
;endwhile
;print,'&&&&&&&&&&&& ',total(counter_specific-counter_all1)
...
t1=SYSTIME(1,/seconds)                   ;to check on speed
 ...
 ;print,'##### SWP2 ',ni,i,edac_on,ptr,nn_e,total(swp2_I2[ni,*]),swp2_I2[ni,20],swp2_I2[ni,110]
...
t1=SYSTIME(1,/seconds)                   ;to check on speed
     ...
     if i EQ 452 then begin
        print,i,' counter ',counter,' ORB MO ', orb_md(i),' MC len ',MC_LEN(i)
        
        print,'$$$$$$$$$$$$$$$$$$$$$$'
        aa=-8
        for iii=-8,40 do begin
        bb=aa+8-1
        dummy             =string(newfile_unsigned[counter+place+offset1-1+aa:counter+place+offset1-1+bb],format = '(z)')
        print,dummy
        aa=bb+1
        endfor
    endif
...
t1=SYSTIME(1,/seconds)                   ;to check on speed
     ...
     FOR ni=0L,nn_ACT_LF-1 do begin
       ...
       counter             = counter_specific[i]   ;get the right counter     
       dummy                = string(newfile_unsigned[counter+place+offset1-1],format = '(B016)')
       reads,dummy,orbmode_dummy,nothing,SMPAVG_dummy,format = '(B04,B09,B03)'
       ORB_MD[i]            = orbmode_dummy
       SMP_AVG[i]           = SMPAVG_dummy   
...
t1=SYSTIME(1,/seconds)                   ;to check on speed
     ...
     FOR ni=0L,nn_PAS_HF-1 do begin
         ...
         counter             = counter_specific[i]   ;get the right counter     
         dummy               = string(newfile_unsigned[counter+place+offset1-1],format = '(B016)')
         reads,dummy,orbmode_dummy,E12_HF_gain_boost_dummy,nothing,SMPAVG_dummy,format = '(B04,B01,B08,B03)'
         ORB_MD[i]           = orbmode_dummy
         SMP_AVG[i]          = SMPAVG_dummy  
         E12_HF_GB[ni]      = E12_HF_gain_boost_dummy 
 ...
 t1=SYSTIME(1,/seconds)                   ;to check on speed      
      FOR ni=0L,nn_HSBM_LF-1 do begin  
         ...
         if compressed EQ 1 then  begin   
           ...
           nn_e    = 0                                              ; this is when the data is not eact a factor of 16     
           ;print, counter, ptr, ptr_end, nn_e
           ...
           endwhile
           ;while ptr_end-2 gt ptr do   decomp = [decomp,mvn_lpw_r_block32(newfile_unsigned,ptr,nn_e,mask16,bin_c,index_arr,32,edac_on)]
...
t1=SYSTIME(1,/seconds)                   ;to check on speed
     FOR ni=0L,nn_htime-1 do begin
         ...
         counter             = counter_specific[i]   ;get the right counter     
      for iii = 0,(((length[i]-1)/2)-7)/2 do begin
          if (p23 EQ 0) AND (iii EQ 0) then cap_time = newfile_signed[counter+place+offset1+2*iii] else  cap_time = [cap_time,newfile_signed[counter+place+offset1+2*iii]]
          ...
          if (p23 EQ 0) AND (iii EQ 0) then htime_type = type_dummy else htime_type = [htime_type, type_dummy]
          if (p23 EQ 0) AND (iii EQ 0) then xfer_time = xfertime_dummy else xfer_time = [xfer_time, xfertime_dummy]
      endfor
      p23 = p23 + 1
...
;Waveform Preallocations   since this is doublicate the waveform1_array I skip this one
...
output=create_struct(   $        ;To export the data in workable form
 ...
 ;  'CCSDS_primary'   ,  CCSDS_primary   ,$
   'APID'            ,  APID            ,$
 ;  'CCSDS_secondary' ,  CCSDS_secondary ,$
   'SC_CLK1'         ,  SC_CLK1         ,$
   'SC_CLK2'         ,  SC_CLK2         ,$
...
;   'waveform5'       ,  waveform5       ,$
 ...
 ;  'ACT_LF_array'    ,  ACT_LF_array    ,$
 ;  'ACT_MF_array'    ,  ACT_MF_array    ,$
 ;  'ACT_HF_array'    ,  ACT_HF_array    ,$
 ;  'PAS_LF_array'    ,  PAS_LF_array    ,$
 ;  'PAS_MF_array'    ,  PAS_MF_array    ,$
 ;  'PAS_HF_array'    ,  PAS_HF_array    ,$
   'HSBM_LF'         ,  hsbm_lf         ,$   
   'HSBM_MF'         ,  hsbm_mf         ,$ 
   'HSBM_HF'         ,  hsbm_hf         ,$
   'xfer_time'       ,  xfer_time       ,$
   ...
   'cap_time'        ,  cap_time        , $
   'SC_CLK1_gst'     ,  SC_CLK1_gst      , $    ; time when the gst stampet the packet
   'SC_CLK2_gst'     ,  SC_CLK2_gst      , $     ; time when the gst stampet the packet
