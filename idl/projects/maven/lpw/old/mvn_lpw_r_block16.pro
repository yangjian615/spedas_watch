...
;----------------------
;
; Decomutater for the compression for block=16 
;
;----------------------
...
function mvn_lpw_r_block16,data,ptr,nn_e,mask16,bin_c,index_arr,arr_size,edac_on
  ...
  for nii = 0,n_loops-1 do begin   ; 8 sets        
   ...
   ; this first part should it be outside this routine????
        ptr_size=(20+9+20*15)/8 < ((n_elements(data)-ptr)-1)
        ...
        ptr_size=(20+9+20*15)/8 < ((n_elements(data)-ptr)-1)
  ;  print,'##### ',nii,n_loops,ptr,ptr_end,n_elements(data),' x ',ptr_size,n_elements(tmp)
        
              
        for ui=1,ptr_size-1  do begin      ; remove this begin end .....
            tmp0[*]=mask16[tmp[ui],*]  ;tmp0(*)=mask16(tmp(ui),*)
            bin=[bin,tmp0]
        endfor       
  ...
  ; print,'AAAA temp ',0,nii,nd,ptr,bin(0+nn_e),' ',temp_16(0)          ,' x ',abs(temp_16(0)),' F ',bin(0+nn_e:19+nn_e),' EN ',bin(20+nn_e:28+nn_e),format='(A,I,I,I,I,I,A,F,A,B20,A,20I1,A,9I1)'         
      
   ;    print,'BBBB',nn_g,nn_e,' nd ',nd, nn_g+14*nd+nd-1,n_elements(bin),nd_1
      IF nn_g+14*nd+nd-1 GE n_elements(bin) then nd=(n_elements(bin)-nn_g)/15
   ;   print,'BBBB',nn_g,nn_e,' nd ',nd, nn_g+14*nd+nd-1,n_elements(bin)
   ...
   ;  print,'$$$$$$ total ',total(output),total(temp_16),temp_16(0),temp_16(1),reject
