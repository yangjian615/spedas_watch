...
;PURPOSE:
;  Decomutater of the LPW telemetry data, this is the old mvn_lpw_r_header which was based on words
;  the new loader is operating on bytes
;  but now include also to work on L0 filemask16
;  r_header orignal written by Corinne Vanatta and David Meyer
;    this L0 reader only work if compression is on!!! use the other r_reader for compression off
;     note APID 62,63,63,64,65,66 will not be found using this routine (lab packets)
...
;
;  3D distributions are stored in 1 to 4 packets, depending on the group 
...
;
;USAGE:
;  mvn_lpw_r_header, filename, output,strip_pad = strip_pad, wrapper = wrapper, compressed=compressed, packet=packet ,no_sc=no_sc
;
;INPUTS:
...
;       no_sc:      When activated then the software behaves as old mvn_lpw_r_header.
;
...
print,'lpw_loader filename ',filename
...
;THIS IS THE NEW LINES FOR WORKING WITH BYTES

; Data files with sc_headers
;********** THIS is where there is a need to flip ***********
...
;***********************************************************
INAPID1=((newfile_byte GE 5*16+0) and (newfile_byte LE 5*16+3)) or ((newfile_byte GE 5*16+5) and (newfile_byte LE 6*16+1)) or((newfile_byte EQ 6*16+7));LPW inst number 50-67,omitting 54,62-66,HSBM has a second check
INAPID2=(newfile_byte GE 5*16+4)  ;LPW inst number HSK 
...
tmp_ss=tmp_ss1+tmp_ss2


nn=n_elements(tmp_ss)
qq=where(tmp_ss(0:nn-8),nq)
print,'Number of packets ',nq
;qq=where(SCAPID(1:*) * INAPID(12:*)*NNAPID(11:*),nq)
...
; print,ni,i,counter,  ORB_MD[i],newfile_byte[ 12L + counter_specific[pkt_HSBM_HF[ni]]+1] ,newfile_byte(counter-25:counter+10),format='(5i,36z)'
...
;print,'##### APID first LF ',where(APID EQ 95),5*16+15
;print,'##### APID first MF ',where(APID EQ 96),6*16+0
;print,'##### APID first HF ',where(APID EQ 97),6*16+1
tmp=where(APID EQ 97,ntmp)
;print,'%%%%% ',newfile_byte(qq(ntmp)+12) ,' *** 84 isapid 0x54 ****, APID(ntmp)
;print,newfile_byte(counter_specific(ntmp)-20:counter_specific(ntmp)+10),format='(31z)'

APID=newfile_byte(qq+12)     +  $                                             ;skipping the 0x information i.e. 08 infomation in newfile_byte(qq+11)
                   1000 *(  (newfile_byte(qq+12) GE 5*16+14) and (newfile_byte(qq+12) LE 6*16+1) and (newfile_byte(qq+12+12) NE 0))   ;Have problem with HSBM -5f,60,and61- and here try to remove that
                   
;print,'##### APID first LF ',where(APID EQ 95)
;print,'##### APID first MF ',where(APID EQ 96)
;print,'##### APID first HF ',where(APID EQ 97)
...
SC_CLK1=2L^24*newfile_byte(qq+17)+2L^16*newfile_byte(qq+18)+2L^8*newfile_byte(qq+19)+newfile_byte(qq+20)     ;seconds clock 32 bits
SC_CLK2=2L^8*newfile_byte(qq+21)+newfile_byte(qq+22)                          ;sub-secongs   16 bits
...
t2=systime(1,/seconds)
Print,' ## Find the APID took ',t2-t1,' seconds ##'
t1=systime(1,/seconds)
...
;counter_specific=counter_all1       ; to replace this variable

 ;this is to make quick conversions for the compressed data
          mvn_lpw_r_mask,mask16,mask8,bin_c,index_arr,flip_8 
          mask16=0. ;;; no longer read the file as wrod, only as byte
...
t1=SYSTIME(1,/seconds)                   ;to check on speed
   ...
   ;  print,'$$$$$ ',counter_specific[pkt_ATR]
     
     FOR ni=0L,nn_ATR-1 do begin    
      
      
   ;   print,newfile_byte[counter_specific[i]:counter_specific[i]+25],format='(26z)'
...
t1=SYSTIME(1,/seconds)                   ;to check on speed
 ...
 ;print,ni,i,counter,newfile_byte(counter-25:counter+10),format='(3i,36z)'
...
;HSK Packet   <---- some signed and some unsigned values
   total_hsk_length = 0
...
;print,'$$$$$$ swp2 test the counter ',nn_SWP2


;----------------  to check counter against time, hardcoded t_epoch
 ; t_epoch = time_double('2000-01-01/12:00:00')                    ; Working for ATLO fall 2012 
;ttime=double(SC_CLK1(pkt_SWP2)+SC_CLK2(pkt_SWP2)/2l^16)+t_epoch
;FOR ni=0L,nn_SWP2-1 do $
;  print,' swp2  ',ni,'  ',time_string(ttime(ni)),' ',ttime(ni), $
;      ' ',newfile_byte(12L + counter_specific[pkt_SWP2(ni)]-10),newfile_byte(12L + counter_specific[pkt_SWP2(ni)]-9),$
;        format='(a,i,a2,a,a1,f24.10,a1,2z2)'
;stanna  
;----------------
...
;HSBM LF packets
 ...
 t1=SYSTIME(1,/seconds)                   ;to check on speed       
  ...
  ;    ptr=ptr+5     
 ; print,'$$$ HSBM LF $$$$$ ',ni,i, counter,' mode ',ORB_MD[i],' dt ',place+offset1,' length ',length[i],(floor(length[i]+1)/2+2+offset2-offset1+1)
; if ni gt 1 then stanna          
  ...
  ;             print,'$$$ HSBM LF $$$$$ ',ni, counter, ' start ',ptr,' end ' , ptr_end,' dp ',ptr_end-ptr,' while ',((ptr_end-ptr)*16-nn_e),(16+7+(31*2)),' memory spilling over', nn_e     
               decomp = [decomp,mvn_lpw_r_block32_byte(newfile_byte,ptr,nn_e,mask8,bin_c,index_arr,32,edac_on)] ;the rest of the times         
           endwhile
           ;while ptr_end-2 gt ptr do   decomp = [decomp,mvn_lpw_r_block32_byte(newfile_byte,ptr,nn_e,mask8,bin_c,index_arr,32,edac_on)]
           p                = ptr_new(decomp)                             
...
print,ni,i,ptr,ptr_end,  ORB_MD[i],newfile_byte[ counter+1] ,newfile_byte(ptr-0:ptr+20),format='(6i,21z)'
if ni gt 5 and ni LT 100 then ni= nn_HSBM_HF-1-3
...
;   wewe=[374,409,501,534,978]
 
;    wewe=[219,339,520,699,717,736,804,938,990,1203,1240]    
    
    
    
; for ii=0,n_elements(HSBM_HF_i)-1 do $
   ;  if total(ii GE 529 and ii LE 666) then   begin
;       if total(HSBM_HF_SC(ii) EQ wewe) then   begin
  
;     tmp1=*hsbm_hf_comp_p(ii-1)
;     tmp2=*hsbm_hf_comp_p(ii)
;     tmp3=*hsbm_hf_comp_p(ii+1)
;          print,ii,pkt_HSBM_HF[ii],12L + counter_specific[pkt_HSBM_HF[ii]],hsbm_hf_i(ii),hsbm_hf_comp_t(ii),HSBM_HF_SC(ii-1),HSBM_HF_SC(ii),HSBM_HF_SC(ii+1),' # ',tmp1(0),tmp1(1),tmp1(2),' # ',tmp2(0),tmp2(1),tmp2(2),' # ',tmp3(0),tmp3(1),tmp3(2)
;     endif
...
t1=SYSTIME(1,/seconds)                   ;to check on speed
     ...
     ;    print,iii,newfile_byte[merge_large(iii*2)],' # ',newfile_byte[merge_small(iii*2)],' D ',tmp(2*iii),' # ',  $
     ;     cap_time2[iii], ' $ ', tmp(2*iii), ' $ ',htime_type2[iii],' # ',xfer_time2[iii]  ,format='(I,b,a,b,a,b,a,b,a,b,a,b,a,b)'
...
if (nn_w1  NE 0) OR (nn_w2  NE 0) OR (nn_w3  NE 0) OR (nn_w4  NE 0) OR (nn_w5  NE 0)  then BEGIN
    print,'This reader cannot read wave pacets, use the original mvn_lpw_r_header without SC headers'
;    stanna
endif 
...
;Fix for missing hsbm_hf, jmm, 2014-07-10
if(n_elements(hsbm_hf) eq 0) then hsbm_hf = !values.f_nan

t2=SYSTIME(1,/seconds)                       ;to check on speed
help,hsbm_lf,hsbm_mf,hsbm_hf, n_elements(hsbm_lf_comp_t), n_elements(hsbm_mf_comp_t), n_elements(hsbm_hf_comp_t)
