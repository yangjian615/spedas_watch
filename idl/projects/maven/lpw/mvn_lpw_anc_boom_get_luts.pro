...
if getenv('mvn_lpw_software') ne '' then begin
      if keyword_set(shadow) then begin
          ...
          fname = fbase+folder+sl+'mvn_lpw_cal_boom_shadow_lut_v01_r01.txt'
      ...
      if keyword_set(wake) then begin
        ;Check we can find the file first, if not, return a nan so it doesn't crash:
        ;fname = '/Users/chfo8135/LASP/MAVEN/LPW_Software/save_files/boom_luts/wake_lut.txt'  ;#### this must be changed to LASP server name
        fname = fbase+folder+sl+'mvn_lpw_cal_boom_wake_lut_v02_r01.txt'
