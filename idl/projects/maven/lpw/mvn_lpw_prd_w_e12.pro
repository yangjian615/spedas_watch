...
pro mvn_lpw_prd_w_E12,make_cdf=make_cdf
...
IF total(strmatch(names, 'mvn_lpw_act_E12')) EQ 1 THEN $ 
  get_data, 'mvn_lpw_act_E12', data=data1,limit=limit1,dlimit=dlimit1 ELSE missing_variable=[missing_variable,'mvn_lpw_act_E12 was not found']

IF total(strmatch(names, 'mvn_lpw_pas_E12')) EQ 1 THEN $
     get_data, 'mvn_lpw_pas_E12', data=data2,limit=limit2,dlimit=dlimit2 ELSE missing_variable=[missing_variable,'mvn_lpw_act_E12 was not found']
...
IF n_elements(missing_variable) GT 1 then print,'mvn_lpw_prd_w_E12: ##### WARNING ###### ',missing_variable
...
IF ((type_1 NE 8) and (type_2 NE 8)) EQ 1 then begin
     print,'mvn_lpw_prd_w_E!2: There was no data found '
     IF min(data1.x) GT max(data2.x) OR min(data2.x) GT max(data1.x) THEN $
                       print,'mvn_lpw_prd_w_E12:### WARNING #### The two time periods do not over lap, check the two tplot variables! '
     IF min(data1.x) GT max(data_sc_att.x) OR min(data_sc_att.x) GT max(data1.x) THEN $
                      print,'mvn_lpw_prd_w_E12:### WARNING #### The SC attitude file are from another time period '                  
     return                                             
ENDIF  
...
;---------------------------------------------------------------------------------------------------
 ...
 IF (type_1 EQ 8) + (type_2 EQ 8) LT 2 THEN BEGIN  ; only one data set
          If (type_2 EQ 8) THEN data1=data2                                 ; get the only information into data1
          nn1         = n_elements(data1.x) 
          data_x     =data1.x
          data_y     =data1.y
          data_dy    =data1.dy
          data_id    =1+fltarr(nn1)+(type_2 EQ 8)                         ;This is to keep track which variable created which data (1,2...)                
          data_l2_flag=fltarr(nn1) 
 ENDIF ELSE BEGIN                                    ;both data sets exist      
...
If (type_sc_att NE 8) then begin
          print,'mvn_lpw_prd_w_E12: #### WARNING no spacecraft attitude data #### set flag to -1.'
          data_l2_flag=-1.         ; no confidence in the data, the probes can be anywhere!!!
ENDIF 

;  ################ to be written ######################## this is where science are made
...
;------------------All information based on the SIS document-------------------         
                ...
                ;-------------------------------------------
                dlimit_l2=create_struct(   $             ; Which are used should follow the SIS document for this variable !! Look at: Table 13: : Contents for LPW.calibrated.w_E12, LPW.calibrated.w_E12_burst_lf, LPW.calibrated.w_E12_burst_mf, and LPW.calibrated.w_E12_burst_hf calibrated data files.
                   ...
                   'cal_source'      ,     'Merging: mvn_lpw_act_E12 and mvn_lpw_pas_E12', $  ; Information of what has been considered in the data production (sc attitude, other instruments etc)       
                   ...
                   ;'zsubtitle'       ,     '[xx]')   ; units                  
                ;-------------------------------------------
                limit_l2=create_struct(   $                ; Which are used should follow the SIS document for this variable !! Look at: Table 13: : Contents for LPW.calibrated.w_E12, LPW.calibrated.w_E12_burst_lf, LPW.calibrated.w_E12_burst_mf, and LPW.calibrated.w_E12_burst_hf calibrated data files.
                ...
                ;---------------------------------------------
                store_data,'mvn_lpw_w_e12_l2',data=data_l2,limit=limit_l2,dlimit=dlimit_l2 
                ;---------------------------------------------    
...
If (keyword_set(make_cdf)) Then $
             mvn_lpw_cdf_write_l2,'mvn_lpw_w_e12_l2'
