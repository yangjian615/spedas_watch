pro mvn_lpw_anc_spacecraft_cf, unix_in, not_quiet=not_quiet
;+
;
;PROCEDURE:   mvn_lpw_spacecraft
;PURPOSE:
...
;
;USAGE:
; mvn_lpw_cruciform, unix_in, et_in, kernel_dir="kernel_dir", /quiet
; 
;INPUTS:
;              
;- unix_in is input for now for labeling purposes and plotting only. It is not used with the SPICE routines
;
;
;OUTPUTS:
...
;May 16th CF: changed name. mvn-lpw-anc-spacecraft now uses Davins SPICE routines. This routine is a copy using my routines.
;-
;=================

;Load kernels:
scid = -202  ;MAVEN s/c ID for SPICE
kernel_version = 'anc_spice_ver V1.'   ;needed for dlimits, add loaded kernels to it
...
if total(strmatch(tplotnames, 'mvn_lpw_load_kernel_files')) eq 1 then begin  ;found kernel tplot variable
    ...
    loaded_kernels=strarr(nele_kernels)  ;string array for dlimits
    for aa = 0, nele_kernels-1 do begin
        ...
        loaded_kernels[aa] = kname  ;copy kernels as a string   
...
for hh = 0, nele_kernels-1 do begin
    ...
    if stregex(dl_kernels.Kernel_files[hh], sl+'maven_v[^tf]*tf', /boolean) eq 1 then fk += 1
...
if min(spkcov) eq 1 then coverage = 'all' else coverage = 'some'
...
cspice_et2utc, et_time, 'ISOC', 3, utc_time  ;convert et to utc time  ### LSK file needed here ###
...
if coverage eq 'all' then cspice_spkpos, target, et_time, frame, abcorr, observer, state, ltime else begin ;state contains R and V [0:5], ltime = light time between observer and object
...
for aa = 0L, nele -1 do begin
    ...
    mvn_abs_angles_z[aa] = acos((mvn_z[0]*vector_sun[aa,0] + mvn_z[1]*vector_sun[aa,1] + mvn_z[2]*vector_sun[aa,2]) / (mag_sun * mag_mvnz)) * (180.D/!pi)   ;in degrees
                ...
                limit=create_struct(   $               
                  ...
                  'noerrorbars', 1)   
               ;---------------------------------
               store_data, 'mvn_lpw_abs_angle_z', data={x:unix_in, y:mvn_abs_angles_z}, dlimit=dlimit, limit=limit
...
ydata[*,1] = mvn_angles[*,1]
                ...
                limit=create_struct(   $               
                  ...
                  'noerrorbars', 1)   
               ;---------------------------------
               store_data, 'mvn_lpw_angles', data={x:unix_in, y:ydata}, dlimit=dlimit, limit=limit
...
;state_j = spice_body_pos(target, observer, utc=utc_time, frame=frame, abcorr=abcorr)  ;Suns position, from MAVEN, in S/C frame

if coverage eq 'all' then cspice_spkpos, target, et_time, frame, abcorr, observer, state_j, ltime else begin ;state contains R and V [0:5], ltime = light time between observer and object
...
sun_pos_j2000[*,2] = state_j[2,*]
                ...
                limit=create_struct(   $               
                  ...
                  'noerrorbars', 1)   
               ;---------------------------------
               store_data, 'mvn_lpw_sun_pos_j2000', data={x:unix_in, y:sun_pos_j2000}, dlimit=dlimit, limit=limit
...
if coverage eq 'all' then cspice_spkezr, target, et_time, frame, abcorr, observer, state, ltime else begin ;state contains R and V [0:5], ltime = light time between observer and object
    state = dblarr(3,nele)  ;must fill this rotation matrix in one time step at a time now. Here time goes in y axis for spice routines
    for aa = 0, nele-1 do begin  ;do each time point individually
...
;mvn_pos_au = mvn_pos_mso / (1.496D8)   ;position in AU
                ...
                limit=create_struct(   $               
                  ...
                  'noerrorbars', 1)   
               ;---------------------------------
               store_data, 'mvn_lpw_pos_J2000', data={x:unix_in, y:mvn_pos_j2000}, dlimit=dlimit, limit=limit
               ;---------------------------------
                ...
                limit=create_struct(   $               
                  ...
                  'noerrorbars', 1)   
               ;---------------------------------             
            ;   store_data, 'mvn_lpw_vel_J2000', data={x:unix_in, y:mvn_vel_j2000}, dlimit=dlimit, limit=limit  ### need velocity added to Davin's spice routines
               ;---------------------------------
...
;if coverage eq 'all' then stateezr = spice_body_pos(target, observer, utc=utc_time, frame=frame, abcorr=abcorr) else begin
if coverage eq 'all' then cspice_spkezr, target, et_time, frame, abcorr, observer, stateezr, ltime else begin ;state contains R and V [0:5], ltime = light time between observer and object
...
;mvn_vel_mso[*,2] = state[5,*]  ;velocities in km
                ...
                limit=create_struct(   $               
                  ...
                  'noerrorbars', 1)   
               ;---------------------------------
               store_data, 'mvn_lpw_pos_mso', data={x:unix_in, y:mvn_pos_mso}, dlimit=dlimit, limit=limit
               ;---------------------------------
                ...
                limit=create_struct(   $               
                  ...
                  'noerrorbars', 1)   
               ;---------------------------------             
            ;   store_data, 'mvn_lpw_vel_mso', data={x:unix_in, y:mvn_vel_mso}, dlimit=dlimit, limit=limit  ;#### no velocity yet
               ;---------------------------------
...
for bb = 0, 2 do ydata[*,6+bb] = mvn_att_j2000[bb,2,*]
                ...
                limit=create_struct(   $               
                  ...
                  'noerrorbars', 1)   
               ;---------------------------------
               store_data, 'mvn_lpw_att_j2000', data={x:unix_in, y:ydata}, dlimit=dlimit, limit=limit
...
for bb = 0, 2 do ydata[*,6+bb] = mvn_att_mso[bb,2,*]
                ...
                limit=create_struct(   $               
                  ...
                  'noerrorbars', 1)   
               ;---------------------------------
               store_data, 'mvn_lpw_att_mso', data={x:unix_in, y:ydata}, dlimit=dlimit, limit=limit
               ;---------------------------------
