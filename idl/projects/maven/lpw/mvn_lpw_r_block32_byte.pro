...
;
;USAGE:
;  mvn_lpw_r_block32_byte,data,ptr,nn_e,mask8,bin_c,index_arr,arr_size,edac_on
;
;INPUTS:
...
function mvn_lpw_r_block32_byte,data,ptr,nn_e,mask8,bin_c,index_arr,arr_size,edac_on
   ...
   ptr_size=(16+7+16*31)/8  < ((n_elements(data)-ptr)-1)  ; the maxumum size length in bytes to request data from and not longer than the data array
         ...
         ELSE $                                                                                                                                ;if nd is not == 1
           ...
           ELSE $                                                                                                                               ; if nd not == 1,2      
             IF nd EQ 16  then $                                                                                                                ;  ND == 16 decoding disable 
               for ii=0,30 do temp_32[ii+1]=   $                                                                                                ; first value is the sign, then sum over the other nd-1 elements                          (1-bin[nn_g+0+ii*nd])* total(double(                                        bin[1+ii*nd+nn_g:15+ii*nd+nn_g]*bin_c[14-index_arr[0:14]]))-        $          ; if the value is positive      
