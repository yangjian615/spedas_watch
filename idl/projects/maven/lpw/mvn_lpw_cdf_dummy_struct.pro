...
;Not a whole lot of error checking here
    ...
    dtime = 'time_unix'   ;this is either 'unix_time' or 'epoch', depending on if the cdf files contain tt2000 time or not
...
time_tt2000 = cdf_parse_tt2000(utc_time)
has_tt2000 = 1.
...
if has_tt2000 eq 1 then begin
    Case(size(time_tt2000, /type)) Of        ;text code for epoch
      ...
      Else: otp = 'Undefined format'
    Endcase
    vtatt.depend_0 = "time_tt2000"  ;must be called this for PDS, does time variable need a depend_0?
    vtatt.catdesc = "CDF TT2000 time."
    vtatt.Var_notes = "Calculated using IDL8.3 'cdf_parse_tt2000' routine."
    vtatt.Fieldnam = 'NA'
  ...
  ;  vtatt.Validmax = 
    ...
    vtstr.name = "time_tt2000"  ;has to be called this for Autoplot to find it
    vtstr.num = n_elements(time_tt2000)
    vtstr.datatype = 'CDF_DOUBLE'
    vtstr.type = 5
    vtstr.ndimen = 1
