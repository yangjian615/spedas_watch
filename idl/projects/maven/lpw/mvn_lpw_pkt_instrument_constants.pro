...
;  keep the common structure so that that software is in operation 
;
;USAGE:
;        mvn_lpw_pkt_instrument_constants,board,lpw_const2=lpw_const2 
;
;INPUTS:
...
;VERSION:      1.2      <------------------------------- update 'version_calibration_routine' variable
;LAST MODIFICATION:   11/01/13
;-
...
version_calib_routine   = '1.2'                     ;keep track of major changes 
tplot_char_size= 1.2
 ;---------------------

print,'(mvn_lpw_pkt_instrument_constants) Selected Board: ',board
...
;--------------- LPW Variables Independend of Board ----------------------- 
   ...
   ;   print,'(mvn_lpw_spectra ) is it correct to take table value * fft_size ???'  
    ...
    f_zero_freq=1./1.     ; is the FPGA code corrected such that all power is not dumped in the zero bin? 
     ...
     calib_file_spec='mvn_lpw_calibration_file_spec_131101_v1.txt'    ; the file needs to be located with the mvn_lpw-procedures to be found      
    ...
    dt_euv=1.0                                           ;sampling rate [sec]   this is only valid for normal operations.....check the number of points averaged....
