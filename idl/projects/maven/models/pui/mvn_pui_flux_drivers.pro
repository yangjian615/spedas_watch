...
if nt eq 1 then begin
  ...
  store_data,'v_(km)',data={x:times,y:[[vxyz],[pui2.vtot]]/1e3},limits={labels:['x','y','z','v'],colors:'bgrk',labflag:1}
  ;  store_data,'R_(km)',data={x:times,y:transpose([pui2.rtot,rcss]/1e3)},limits={colors:'br',labels:['Mars','CSS'],labflag:1}
  store_data,'n_(/cm3)',data={x:times,y:[[nnh],[nno]]},limits={labels:['H','O','CSS'],colors:'brg',labflag:1,ylog:1}
  ...
  store_data,'dn_(/cm3)',data={x:times,y:dnnn},limits={ylog:1}
  store_data,'cumulative_n_(/cm3)',times,nnn
...
;  p=plot([0],/nodata,/ylog,yrange=[10,1e6],xtitle='Pickup Ion Energy (keV)',ytitle='Differential Flux (/[cm2 s keV])')
;  p=plot(pui2.ke/1e3,1e3*dphi/abs(pui2.de),/o,'b')
;  p=plot(kflux,/o,'r')
