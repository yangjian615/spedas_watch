...
function average_hist2,a,x,binsize=binsize,trange=trange,centertime=centertime
  ...
  sizea=size(a)
  sizeh=size(h)
  if (sizea[0] eq 1) then avg=replicate(1.,sizeh[1])
  if (sizea[0] eq 2) then avg=replicate(1.,sizeh[1],sizea[2])
  if (sizea[0] eq 3) then avg=replicate(1.,sizeh[1],sizea[2],sizea[3])
  if (sizea[0] eq 4) then avg=replicate(1.,sizeh[1],sizea[2],sizea[3],sizea[4])
  if (sizea[0] eq 5) then avg=replicate(1.,sizeh[1],sizea[2],sizea[3],sizea[4],sizea[5])
