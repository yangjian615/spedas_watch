...
;check for slashes, add if necessary
  temp_string = strtrim(!maven_spd.local_data_dir, 2)
  ll = strmid(temp_string, strlen(temp_string)-1, 1)
  If(ll Ne '/' And ll Ne '\') Then temp_string = temp_string+'/'
  !maven_spd.local_data_dir = temporary(temp_string)
  temp_string = strtrim(!maven_spd.remote_data_dir, 2)
  ll = strmid(temp_string, strlen(temp_string)-1, 1)
  If(ll Ne '/' And ll Ne '\') Then temp_string = temp_string+'/'
  !maven_spd.remote_data_dir = temporary(temp_string)
