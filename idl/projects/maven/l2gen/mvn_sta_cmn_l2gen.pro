...
;Case by case basis
        Case vj of
           ...
           End        
           'natt': Begin
              dvar = fix(n_elements(uniq(cmn_dat.att_ind)))
           End
           'nswp': Begin
              dvar = fix(n_elements(uniq(cmn_dat.swp_ind)))
           End
           'neff': Begin
              dvar = fix(n_elements(uniq(cmn_dat.eff_ind)))
           End
           'nmlut': Begin
              dvar = fix(n_elements(uniq(cmn_dat.mlut_ind)))
           End
           'dead_time_1': Begin
              dvar = cmn_dat.dead1
           End
...
;Create filename and call cdf_save_vars.
  ...
  ext = strcompress(strlowcase(cmn_dat.apid), /remove_all)+'-'+estring+mstring+dstring+astring
