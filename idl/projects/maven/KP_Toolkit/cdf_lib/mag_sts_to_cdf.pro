...
PRO mag_sts_to_cdf, filename
  ...
  ;str_element, data_asc, 'time_unix', time_unix, /add



  ;-----------------------------------
  ;Repackage MAG components into a vector
  ;bvec = fltarr(n_elements(data_asc.x), 3)
  ;bvec[*,0] = data_asc.x
  ;bvec[*,1] = data_asc.y
  ;bvec[*,2] = data_asc.z
  ;str_element, data_asc, 'data_asc', bvec, /add
  ...
  ;----------------------------
  ;Create format info .txt file
  ;openw, 1, strmid(file,0,sts_pos[0])+'.txt'
  ;for i=0., n_elements(obj_str)-1 do printf, 1, obj_str[i]
  ;close, 1
